...
def manage_counter_and_check_linters(tool_name, input_data):
    ...
    # Check if we should run linters (every 10th write operation for that specific type)
    should_run = current_count % 10 == 0 
        ...
        if should_run_linters:
            ...
            if linter_results or should_run_linters:
                ...
                linter_output.append(
                    f"🔍 LINTER CHECK{linter_type_label} (every 10th {linter_operation_type or ''} write operation, #{current_count})"
                )
                linter_output.append(f"{'='*60}")
