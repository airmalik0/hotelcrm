...
export const CustomersService = {
  ...
  updateCustomer: async ({
    customerId,
    requestBody,
  }: { customerId: string; requestBody: any }) => {
...
export const BookingsService = {
  readBookings: async ({
    ...
    skip = 0,
    status,
  }: { limit?: number; skip?: number; status?: string }) => {
    const params = new URLSearchParams({
      limit: String(limit),
      skip: String(skip),
    })
  ...
  updateBooking: async ({
    bookingId,
    requestBody,
  }: { bookingId: string; requestBody: any }) => {
...
export const LoginService = {
  loginAccessToken: async ({
    ...
    formData: { username: string; password: string; grant_type: string }
  }) => {
    const params = new URLSearchParams({
